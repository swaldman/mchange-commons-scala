...
/**
  * For now, this stuff is maintained in a scala language fork, proposed for
  * inclusion within Either.
  * 
  * Modifications/improvements should be made there, and then pulled back here.
  */ 
object EitherAsMonad {
  ...
  final object RightBias {
    ...
    object withEmptyToken {
      abstract class AbstractOps[A,B]( target : Either[A,B] )( opsTypeClass : EitherAsMonad.RightBias.withEmptyToken.Generic[A] ) {
      ...
      implicit final class Ops[A,B]( target : Either[A,B] )( implicit opsTypeClass : EitherAsMonad.RightBias.withEmptyToken.Generic[A] ) extends AbstractOps( target )( opsTypeClass );

      trait Generic[+E] extends EitherAsMonad.Bias[E] {
        /*
  ...
  trait RightBias[L] {
    val EmptyTokenDefinition : EitherAsMonad.RightBias.withEmptyToken.Generic[L] = RightBias.DefaultThrowingOps;
  ...
  final object LeftBias {
    ...
    object withEmptyToken {
      abstract class AbstractOps[A,B]( target : Either[A,B] )( opsTypeClass : EitherAsMonad.LeftBias.withEmptyToken.Generic[B] ) {
      ...
      implicit final class Ops[A,B]( target : Either[A,B] )( implicit opsTypeClass : EitherAsMonad.LeftBias.withEmptyToken.Generic[B] ) extends AbstractOps( target )( opsTypeClass );
  ...
  trait LeftBias[R] { // we use R rather than B here to emphasize to users that it is the right-side type that must be specified here
    val EmptyTokenDefinition : EitherAsMonad.LeftBias.withEmptyToken.Generic[R] = LeftBias.DefaultThrowingOps;
